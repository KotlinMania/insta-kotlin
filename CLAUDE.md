# Claude Code Project Instructions — insta-kotlin

## Project Overview

This is **insta-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`insta`](https://crates.io/crates/insta).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/insta/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/insta>
